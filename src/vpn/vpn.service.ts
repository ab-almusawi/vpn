...
export class VpnService {
  ...
  async fixKeyMismatch(deviceId: string, newPublicKey: string): Promise<{
    success: boolean;
    message: string;
    clientInfo?: {
      deviceId: string;
    ...
    try {
      ...
      if (!client) {
        throw new BadRequestException(`Client with device ID ${deviceId} not found`);
      }

      // Store old public key for removal
      const oldPublicKey = client.publicKey;

      // Remove old peer from server if it exists
      if (oldPublicKey && oldPublicKey !== newPublicKey) {
        try {
          await this.wireguardService.removePeerFromServer(client);
          console.log(`Removed old peer for client ${deviceId}`);
        } catch (error) {
      ...
      const serverPublicKey = await this.wireguardService.getServerPublicKey();
      const serverEndpoint = `${this.configService.get('VPN_SERVER_PUBLIC_IP')}:${this.configService.get('VPN_SERVER_PORT', 51820)}`;
      const configTemplate = await this.wireguardService.generateClientConfig(savedClient);

      console.log(`✅ Key mismatch fixed for client ${deviceId}`);

      return {
        success: true,
        message: 'Key mismatch fixed successfully. Old peer removed and new peer added.',
        clientInfo: {
          deviceId: savedClient.deviceId,
          deviceName: savedClient.deviceName,
          vpnIp: savedClient.vpnIp,
        ...
        serverConfig: {
          serverPublicKey,
          serverEndpoint,
          assignedIp: savedClient.vpnIp,
          dns: '8.8.8.8, 8.8.4.4',
        },
        configTemplate,
      };
    } catch (error) {
      console.error(`❌ Failed to fix key mismatch for ${deviceId}:`, error);
      throw new BadRequestException(`Failed to fix key mismatch: ${error.message}`);
    }
  }

  async updateClientPublicKey(deviceId: string, newPublicKey: string): Promise<Client> {
    const client = await this.clientRepository.findOne({
      where: { deviceId },
    });
    ...
    // Remove old peer if key is changing
    if (client.publicKey && client.publicKey !== newPublicKey) {
      try {
        await this.wireguardService.removePeerFromServer(client);
      } catch (error) {
        console.warn(`Failed to remove old peer: ${error.message}`);
  ...
  async removeClientPeer(deviceId: string): Promise<{ success: boolean; message: string }> {
    try {
      const client = await this.clientRepository.findOne({
        where: { deviceId },
      });
      ...
      return {
        success: true,
        message: `Peer for client ${deviceId} removed successfully`,
      };
    } catch (error) {
      console.error(`Failed to remove peer for ${deviceId}:`, error);
      throw new BadRequestException(`Failed to remove peer: ${error.message}`);
    }
  }
}
